...
app.post("/api/cart", (req, res) => {
  ...
  db.run(
    ...
    (userId, title, concentration, volume, packagingType, price, topNotes, heartNotes, baseNotes, packageImage)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `,
    [
      userId,
    ...
    function (err) {
      if (err) {
        return res.status(500).json({ message: "Could not save cart item." });
      }

      res.json({ id: this.lastID, message: "Cart item saved." });
    }
  );
});
...
app.get("/api/cart/:userId", (req, res) => {
  db.get(
    `SELECT * FROM cart_items WHERE userId = ? ORDER BY createdAt DESC LIMIT 1`,
    [req.params.userId],
    (err, row) => {
      if (err) {
        return res.status(500).json({ message: "Database error." });
...
app.delete("/api/cart/:id", (req, res) => {
  db.run(
    ...
    function (err) {
      if (err) {
        return res.status(500).json({ message: "Could not delete cart item." });
      }
