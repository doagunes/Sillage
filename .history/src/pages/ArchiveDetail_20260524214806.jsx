...
import defaultPerfume from "../assets/perfumes/perfume-1.png";
import saltPerfume from "../assets/perfumes/perfume-2.png";
import cherryPerfume from "../assets/perfumes/perfume-20.png";
import floralPerfume from "../assets/perfumes/perfume-3.png";
import darkPerfume from "../assets/perfumes/perfume-4.png";
...
function DynamicMemoryDetail({ memory }) {
  ...
  const topNotes = memory.topNotes ? memory.topNotes.split(",") : [];
  const heartNotes = memory.heartNotes ? memory.heartNotes.split(",") : [];
  const baseNotes = memory.baseNotes ? memory.baseNotes.split(",") : [];

  const perfumeBottle = getPerfumeBottleByTitle(title);
  ...
  return (
    <main className="dynamic-detail-wrapper">
      <section className="dynamic-detail-panel">
        ...
        <div className="dynamic-perfume-area">
          <div className="dynamic-bottle-wrapper">
            <img
              src={perfumeBottle}
              alt={title}
              className="dynamic-perfume-bottle"
            />

            <div className="dynamic-bottle-label">
              <h2>sillage</h2>
              <p>{title}</p>

              <div className="dynamic-bottle-meta">
                <span>
                  production date
                  <br />
                  2026
              ...
              <small>
                ...
                <br />
                {getMainFragranceDescription(topNotes, heartNotes, baseNotes)}
              </small>
            </div>
          </div>
        </div>
        ...
        <div className="dynamic-info-card">
          ...
          <div className="dynamic-info-body">
            <div className="dynamic-memory-photo">
              <img src={memory.memoryImage} alt={title} />
            </div>

            <div className="dynamic-notes">
              <NoteGroup title="Top Notes" notes={topNotes} />
              <NoteGroup title="Heart Notes" notes={heartNotes} />
...
function getMainFragranceDescription(topNotes, heartNotes, baseNotes) {
  ...
  return allNotes.slice(0, 2).join(" & ");
}

function getPerfumeBottleByTitle(title) {
  ...
  if (
    lowerTitle.includes("cherry") ||
    lowerTitle.includes("blossom") ||
    lowerTitle.includes("rose") ||
    lowerTitle.includes("flower")
  ) {
    return cherryPerfume;
  }
